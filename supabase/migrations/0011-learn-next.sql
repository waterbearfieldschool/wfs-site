...
--
-- Asked of everyone at registration, optional, and repeated as a reply prompt in
-- the confirmation email for anyone who skipped it. The answers are three things
-- at once: the demand signal for next month's programming, the content of the
-- newsletter, and the thing that turns someone who attended into someone who is
-- helping decide what happens next.
--
-- Same p_meta pattern as teach_offer — a new field is one column and one line,
-- and no caller has to change.

alter table public.rsvps
  add column if not exists learn_next text;
...
begin
  ...
  values (p_name, p_email, lbl, p_date,
          ...
          nullif(left(trim(coalesce(p_meta->>'learn_next', '')), 600), ''));
